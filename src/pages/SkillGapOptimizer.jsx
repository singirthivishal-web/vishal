          ...
          <div className="bg-surface border border-surfaceHighlight rounded-xl h-40 p-4 overflow-y-auto flex flex-col gap-3 min-h-[16rem]">
            {!result && !isOptimizing && (
              <div className="flex-1 flex items-center justify-center text-textMuted text-sm">
                Optimized results will appear here.
              </div>
            )}
            {isOptimizing && (
              <div className="flex-1 flex flex-col items-center justify-center space-y-3">
                <div className="flex gap-1">
                  ...
                  <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-2 h-2 rounded-full bg-primary" />
                  <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} className="w-2 h-2 rounded-full bg-primary" />
                </div>
            ...
            {result && result.map((item, index) => (
              ...
              >
                ...
                <button className="absolute right-3 top-1/2 -translate-y-1/2 text-textMuted group-hover:text-primary transition-colors">
